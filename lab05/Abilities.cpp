...
AbilityList &AbilityList::operator=(const AbilityList &source)
{
    ...
    if (this != &source) {
        deleteList();
        this->front = new AbilityNode();
        AbilityNode *tempOther = source.front;
        AbilityNode *tempThis = front;
        while (tempOther->next != NULL) {
        tempThis->next = new AbilityNode();
        tempThis->next->ability.weapon = tempOther->next->ability.weapon;
        tempThis->next->ability.strength = tempOther->next->ability.strength;
...
void AbilityList::deleteList()
{
    ...
    if (front->next == NULL) {
        ...
        AbilityList *temp = new AbilityList();
